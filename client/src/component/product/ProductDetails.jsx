...
const ProductDetails = () => {
    ...
    const {CartForm, CartFormOnChange,CartFormRequest,CartListRequest} = CartStore();
    const {WishFormRequest,WishListRequest} = WishStore();
    ...
    const AddCart = async (productID) => {
        let res=await CartFormRequest(CartForm,productID,quantity);
        if(res){
    ...
    const AddWish = async (productID) => {
        let res=await WishFormRequest(productID);
        if(res){
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-5 p-3">
                            ...
                            <span>
                                {
                                    Details[0]['discount'] ? (
                                        <span className="bodyXLarge">Price: <strike
                                            className="text-secondary">{Details[0]['price']}</strike> {Details[0]['discountPrice']} </span>
                                    ) : (
                            ...
                            <div className="row">
                                <div className="col-4 p-2">
                                    ...
                                    <select onChange={(e)=>{CartFormOnChange('size',e.target.value)}} className="form-control my-2 form-select">
                                ...
                                <div className="col-4 p-2">
                                    ...
                                    <select onChange={(e)=>{CartFormOnChange('color',e.target.value)}} className="form-control my-2 form-select">
                                ...
                                <div className="col-4 p-2">
                                    <CartButton onClick={async ()=>{await AddCart(Details[0]['_id'])}}  className="btn w-100 btn-success" text="Add to Cart"/>
                                </div>
                                <div className="col-4 p-2">
                                    <WishButton onClick={async ()=>{await AddWish(Details[0]['_id'])}}  className="btn w-100 btn-success" text="Add to wish"/>
                                </div>
                            </div>
                        </div>
                    </div>
